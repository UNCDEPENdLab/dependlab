...
score_bfi <- function(df, item_prefix="BFI_", max_impute=0.2, drop_items=FALSE,
  ...
  #compute alphas
  if (add_alphas) {
    attr(df$BFI_soci,"alpha") <- summary(psych::alpha(df[,soci_items],max=100,warnings = F))
    attr(df$BFI_assrt,"alpha") <- summary(psych::alpha(df[,assrt_items],max=100,warnings = F))
    attr(df$BFI_enrgy,"alpha") <- summary(psych::alpha(df[,enrgy_items],max=100,warnings = F))
    attr(df$BFI_comp,"alpha") <- summary(psych::alpha(df[,comp_items],max=100,warnings = F))
    attr(df$BFI_respect,"alpha") <- summary(psych::alpha(df[,respect_items],max=100,warnings = F))
    attr(df$BFI_trust,"alpha") <- summary(psych::alpha(df[,trust_items],max=100,warnings = F))
    attr(df$BFI_org,"alpha") <- summary(psych::alpha(df[,org_items],max=100,warnings = F))
    attr(df$BFI_prod,"alpha") <- summary(psych::alpha(df[,prod_items],max=100,warnings = F))
    attr(df$BFI_respon,"alpha") <- summary(psych::alpha(df[,respon_items],max=100,warnings = F))
    attr(df$BFI_anx,"alpha") <- summary(psych::alpha(df[,anx_items],max=100,warnings = F))
    attr(df$BFI_dep,"alpha") <- summary(psych::alpha(df[,dep_items],max=100,warnings = F))
    attr(df$BFI_emo_vol,"alpha") <- summary(psych::alpha(df[,emo_vol_items],max=100,warnings = F))
    attr(df$BFI_int_cur,"alpha") <- summary(psych::alpha(df[,int_cur_items],max=100,warnings = F))
    attr(df$BFI_aes_sens,"alpha") <- summary(psych::alpha(df[,aes_sens_items],max=100,warnings = F))
    attr(df$BFI_creat_imag,"alpha") <- summary(psych::alpha(df[,creat_imag_items],max=100,warnings = F))
    attr(df$BFI_extra,"alpha") <- summary(psych::alpha(df[,extra_items],max=100,warnings = F))
    attr(df$BFI_agree,"alpha") <- summary(psych::alpha(df[,agree_items],max=100,warnings = F))
    attr(df$BFI_cons,"alpha") <- summary(psych::alpha(df[,cons_items],max=100,warnings = F))
    attr(df$BFI_neg_emo,"alpha") <- summary(psych::alpha(df[,neg_emo_items],max=100,warnings = F))
    attr(df$BFI_open_mind,"alpha") <- summary(psych::alpha(df[,open_mind_items],max=100,warnings = F))
  }
