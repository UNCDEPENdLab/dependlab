...
#' @param col_email_name name of the column which contains the email id for subject to be used when name doesn't match 
#' 
...
#'   match_subIDs_names(tracker_df, screener_df, col_first_name = "ContactForm_1", col_last_name = "ContactForm_2", col_email_name = "ContactForm_3")
#' }
#' 
#' @export 
#' @author Nidhi Desai
...
#' @importFrom magrittr %>% 
#' 
match_subIDs_names <- function(tracker_df, addto_df, col_first_name = "FIRSTNAME", col_last_name = "LASTNAME", col_email_name = ""){
  
  # create a list of names for the subIDs -----------------------------------
  subIDs_names <- tracker_df %>% 
    select(ID, EMAIL, `FIRSTLAST NAME                   (signed consent with)`) %>%
  ...
  for (i in c(1:length(subIDs_names$ID))){
    ...
    subIDs_names$firstName[i] <- tolower(trimws(name_string[1]))
    subIDs_names$lastName[i] <- tolower(trimws(ifelse(length(name_string) == 2, name_string[2], 
                                              ifelse(length(name_string) == 3, name_string[3], NA))))
    subIDs_names$email[i] <- tolower(trimws(subIDs_names$EMAIL[i]))
  }
  # TODO add small case
  # using email to refer when name doesn't match (case of preferred name)
  # multiple names in last name issue
  # Prefered name added to the first name
  
  
  # match IDs based on names and add to file --------------------------------
  addto_df$ID <- NA
  firstName_to_match <- tolower(trimws(addto_df[[col_first_name]]))
  lastName_to_match <- tolower(trimws(addto_df[[col_last_name]]))
  email_to_match <- tolower(trimws(addto_df[[col_email_name]]))
  ...
  for (j in c(1:length(subIDs_names$ID))){
    ...
    } else { # no match for name found
      
      if (nchar(col_email_name) > 0) # column name for email is provided, use email to match 
        matched_nrow <- which(email_to_match == subIDs_names$email[j])
        if(length(matched_nrow) > 0){
          addto_df$ID[matched_nrow[1]] <- subIDs_names$ID[j]
        }
    }
