...
score_pid5 <- function(df, item_prefix="PID_", max_impute=0.2, drop_items=FALSE,
  ...
  #compute alphas
  if (add_alphas) {
    attr(df$PID_anh,"alpha") <- summary(psych::alpha(df[,anh_items],max=100,warnings = F))
    attr(df$PID_anx,"alpha") <- summary(psych::alpha(df[,anx_items],max=100,warnings = F))
    attr(df$PID_attn,"alpha") <- summary(psych::alpha(df[,attn_items],max=100,warnings = F))
    attr(df$PID_callous,"alpha") <- summary(psych::alpha(df[,callous_items],max=100,warnings = F))
    attr(df$PID_deceit,"alpha") <- summary(psych::alpha(df[,deceit_items],max=100,warnings = F))
    attr(df$PID_depres,"alpha") <- summary(psych::alpha(df[,depres_items],max=100,warnings = F))
    attr(df$PID_distr,"alpha") <- summary(psych::alpha(df[,distr_items],max=100,warnings = F))
    attr(df$PID_ecc,"alpha") <- summary(psych::alpha(df[,ecc_items],max=100,warnings = F))
    attr(df$PID_emo_lab,"alpha") <- summary(psych::alpha(df[,emo_lab_items],max=100,warnings = F))
    attr(df$PID_grnd,"alpha") <- summary(psych::alpha(df[,grnd_items],max=100,warnings = F))
    attr(df$PID_host,"alpha") <- summary(psych::alpha(df[,host_items],max=100,warnings = F))
    attr(df$PID_impuls,"alpha") <- summary(psych::alpha(df[,impuls_items],max=100,warnings = F))
    attr(df$PID_int_avd,"alpha") <- summary(psych::alpha(df[,int_avd_items],max=100,warnings = F))
    attr(df$PID_irr,"alpha") <- summary(psych::alpha(df[,irr_items],max=100,warnings = F))
    attr(df$PID_man,"alpha") <- summary(psych::alpha(df[,man_items],max=100,warnings = F))
    attr(df$PID_perc_dysreg,"alpha") <- summary(psych::alpha(df[,perc_dysreg_items],max=100,warnings = F))
    attr(df$PID_persev,"alpha") <- summary(psych::alpha(df[,persev_items],max=100,warnings = F))
    attr(df$PID_rest_aff,"alpha") <- summary(psych::alpha(df[,rest_aff_items],max=100,warnings = F))
    attr(df$PID_perf,"alpha") <- summary(psych::alpha(df[,perf_items],max=100,warnings = F))
    attr(df$PID_rt,"alpha") <- summary(psych::alpha(df[,rt_items],max=100,warnings = F))
    attr(df$PID_sep_insec,"alpha") <- summary(psych::alpha(df[,sep_insec_items],max=100,warnings = F))
    attr(df$PID_sub,"alpha") <- summary(psych::alpha(df[,sub_items],max=100,warnings = F))
    attr(df$PID_sus,"alpha") <- summary(psych::alpha(df[,sus_items],max=100,warnings = F))
    attr(df$PID_unusual,"alpha") <- summary(psych::alpha(df[,unusual_items],max=100,warnings = F))
    attr(df$PID_wthdrwl,"alpha") <- summary(psych::alpha(df[,wthdrwl_items],max=100,warnings = F))
    attr(df$PID_neg_aff,"alpha") <- summary(psych::alpha(df[,neg_aff_items],max=100,warnings = F))
    attr(df$PID_detach,"alpha") <- summary(psych::alpha(df[,detach_items],max=100,warnings = F))
    attr(df$PID_antag,"alpha") <- summary(psych::alpha(df[,antag_items],max=100,warnings = F))
    attr(df$PID_disinhib,"alpha") <- summary(psych::alpha(df[,disinhib_items],max=100,warnings = F))
    attr(df$PID_psycho,"alpha") <- summary(psych::alpha(df[,psycho_items],max=100,warnings = F))
  }
