...
#' @param drop_octants whether to remove the IIP octant scores. Default: FALSE
...
#'
score_iip90 <- function(df, item_prefix="IIP_", max_impute=0.2, drop_items=FALSE,
                        drop_octants=FALSE, min_value=0, max_value=4, add_alphas=TRUE) {
  ...
  #compute row means
  df <- df %>% mutate(IIP_pa = rowMeans(across(all_of(pa_items))),
    ...
    mutate(IIP_c = rowMeans(across(all_of(c_scales))),
  ...
  #compute alphas
  if(add_alphas) {
    attr(df$IIP_pa,"alpha") <- summary(psych::alpha(df[,pa_items],max=100,warnings = F))
    attr(df$IIP_bc,"alpha") <- summary(psych::alpha(df[,bc_items],max=100,warnings = F))
    attr(df$IIP_de,"alpha") <- summary(psych::alpha(df[,de_items],max=100,warnings = F))
    attr(df$IIP_fg,"alpha") <- summary(psych::alpha(df[,fg_items],max=100,warnings = F))
    attr(df$IIP_hi,"alpha") <- summary(psych::alpha(df[,hi_items],max=100,warnings = F))
    attr(df$IIP_jk,"alpha") <- summary(psych::alpha(df[,jk_items],max=100,warnings = F))
    attr(df$IIP_lm,"alpha") <- summary(psych::alpha(df[,lm_items],max=100,warnings = F))
    attr(df$IIP_no,"alpha") <- summary(psych::alpha(df[,no_items],max=100,warnings = F))
    attr(df$IIP_bpd,"alpha") <- summary(psych::alpha(df[,bpd_items],max=100,warnings = F))
    attr(df$IIP_sensitivity_pd1,"alpha") <- summary(psych::alpha(df[,sensitivity_pd1_items],max=100,warnings = F))
    attr(df$IIP_ambivalence_pd2,"alpha") <- summary(psych::alpha(df[,ambivalence_pd2_items],max=100,warnings = F))
    attr(df$IIP_aggression_pd3,"alpha") <- summary(psych::alpha(df[,aggression_pd3_items],max=100,warnings = F))
    attr(df$IIP_approval_c1,"alpha") <- summary(psych::alpha(df[,aggression_pd3_items],max=100,warnings = F))
    attr(df$IIP_lacksocial_c2,"alpha") <- summary(psych::alpha(df[,aggression_pd3_items],max=100,warnings = F))
    attr(df$IIP_c,"alpha") <- summary(psych::alpha(df[,c_scales],max=100,warnings = F))
    attr(df$IIP_pd,"alpha") <- summary(psych::alpha(df[,pd_scales],max=100,warnings = F))
  }

  #drop octants and item-level data
  if (drop_octants) { df <- df %>% select(-IIP_pa, -IIP_bc, -IIP_de, -IIP_fg, -IIP_hi, -IIP_jk, -IIP_lm, -IIP_no) }
